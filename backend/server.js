...
const allowedOrigins = [
  "https://waste-managmen-lpu.vercel.app",
  "http://localhost:5000",
  "http://127.0.0.1:5000",
...
app.use(cors({
  origin: function(origin, callback) {
    ...
    if (allowedOrigins.indexOf(origin) === -1) {
      // If the origin is not in the list, still allow it for local development purposes
...
app.get("/admin/workers", (req, res) => {
  res.sendFile(path.join(__dirname, "..", "frontend", "admin", "workers", "index.html"));
});

app.get("/admin/users", (req, res) => {
  res.sendFile(path.join(__dirname, "..", "frontend", "admin", "users", "index.html"));
});
